...
module.exports = function(app, databaseMongo) {
    ...
    app
        .route('/login').post(passport.authenticate('local', { failureRedirect: '/' }), (req, res) => {
    ...
    app
        .route('/chat').get(ensureAuthenticated, (req, res) => {
    ...
    app
        .route('/register').post(
            (req, res, next) => {
                ...
                databaseMongo.findOne({ username: req.body.username }, function(err, user) {
                    if (err) {
                        next(err);
                    } else if (user) {
                        res.redirect('/');
                    } else {
                        databaseMongo.insertOne({ username: req.body.username, password: hash }, (err, doc) => {
                            if (err) {
                                res.redirect('/');
                            } else {
                                next(null, doc.ops[0]);
